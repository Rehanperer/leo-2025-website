...
interface TeamMember {
    ...
    name: string;
    role: string;
    category: string; // Exco or Director
}
            ...
            <div className="max-w-6xl mx-auto">
                ...
                {loading ? (
                    ...
                    <div className="text-center py-20 bg-gray-900 rounded-xl border border-gray-800">
                        ...
                        <p className="text-sm text-gray-600">Click "Add Member" to build the team.</p>
                    </div>
                ) : (
                    <div className="grid gap-4">
                        {members.map((member) => (
                            <div key={member.id} className="bg-gray-900 border border-gray-800 p-4 rounded-xl flex justify-between items-center">
                                <div>
                                    <h3 className="font-bold text-lg text-white">{member.name}</h3>
                                    <p className="text-sm text-gray-400">{member.role} • <span className="text-brand-cyan">{member.category}</span></p>
                                </div>
                                <button
